...
function StarRating({
  ...
  return (
    <View
      testID={`movie-rating-${movieId}`}
      style={{ flexDirection: "row", alignItems: "center", marginTop: 2 }}
    >
      {Array.from({ length: fullStars }, (_, i) => (
        <Ionicons key={`full-${i}`} name="star" size={14} color="#992576" />
      ))}
      {hasHalf && <Ionicons name="star-half" size={14} color="#992576" />}
    </View>
  );
}
  ...
  const renderItem = useCallback(
    ({ item }: { item: Movie }) => (
      <MovieItem
        ...
        onRank={() =>
          router.push({ pathname: "/comparison", params: { movieId: item.id } })
        }
      />
    ),
    [router],
